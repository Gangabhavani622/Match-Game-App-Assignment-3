...
class App extends Component {
  ...
  getGameProgressPage = (imageUrl, category) => (
    <img src={imageUrl} alt={category} className="random" />
  )
  ...
  renderImageList = filteredImagesList => {
    ;<ul>
      {filteredImagesList.map(eachItem => (
        <ThumbnailItem key={eachItem.id} thumbnailItem={eachItem} />
      ))}
    </ul>
  }

  render() {
    const {score, seconds, isGameProgress} = this.state

    const filteredImagesList = this.getFilteredList()
    const randomIndex = this.getRandomImageIndex()
    const {imageUrl, category} = imagesList[randomIndex]
    return (
      <div className="app-container">
        ...
        <div className="sub-app">
          {isGameProgress ? (
            <div>
              {this.getGameProgressPage(imageUrl, category)}
              {this.renderTabList()}
              {this.renderImageList(filteredImagesList)}
            </div>
          ) : (
            <MatchGameResult score={score} />
          )}
        </div>
      </div>
    )
